...
void Splitting_Triangle_Horizontal(Partial_Solutions* emnt1, Partial_Solutions* emnt2, double Bound_Half, Partial_Solutions* emnt3) {
    ...
    Extra_Constraints.add(obj_fun[1] <= Bound_Half);
    ...
    if (Infeasible_Index == false) {
        double Slope(0), Obj_Value(0);
        if (fabs(emnt2->ObjVal[1] - NewNDP[1]) > Epsilon && fabs(emnt2->ObjVal[0] - NewNDP[0]) > Epsilon) {
            Slope = (emnt2->ObjVal[1] - NewNDP[1]) / (emnt2->ObjVal[0] - NewNDP[0]);
            Obj_Value = Slope * (NewNDP[0]-((2 * (Compute_Epsilon3(NewNDP[0])))) - NewNDP[0]);
            Obj_Value += NewNDP[1];
        } else {
            Obj_Value = Negative_infinity;
        }
        if (Obj_Value < Bound_Half - Epsilon) {
            Extra_Constraints.add(obj_fun[0] <= (NewNDP[0] - Compute_Epsilon2(NewNDP[0])));
            Extra_Constraints.add(obj_fun[1] >= Bound_Half);
            objfunc = IloMinimize(env, obj_fun[1]);
            model.add(objfunc);
            model.add(Extra_Constraints);
        ...
        gap_points1 = fabs(z11 - z21);
        gap_points2 = fabs(z12 - z22);
        if (emnt3->ObjVal[1] <= emnt1->ObjVal[1] + Compute_Epsilon(emnt1->ObjVal[1]) && emnt3->ObjVal[0] <= NewNDP[0]+ Compute_Epsilon(NewNDP[0])) {
        ...
        if (DMValue2 < GUB - Compute_Epsilon(GUB)) {
            int x = 0;
            Diff_Point_Proof[0][0] = fabs(emnt1->ObjVal[0] - element2->ObjVal[0]) / (fabs(element2->ObjVal[0]));
            Diff_Point_Proof[0][1] = fabs(emnt1->ObjVal[1] - element2->ObjVal[1]) / (fabs(element2->ObjVal[1]));

            if (Diff_Point_Proof[0][0] > Epsilon7 && Diff_Point_Proof[0][1] > Epsilon7) {
                Priority_Queue* emnt3 = new Priority_Queue;
                emnt3->Assign_Element_to_Queue(emnt1, element2, false, true, DMValue2);
        ...
        if (DMValue1 < GUB - Compute_Epsilon(GUB)) {
            int x = 0;
            Diff_Point_Proof[1][0] = fabs(emnt2->ObjVal[0] - element1->ObjVal[0]) / (fabs(element1->ObjVal[0]));
            Diff_Point_Proof[1][1] = fabs(emnt2->ObjVal[1] - element1->ObjVal[1]) / (fabs(element1->ObjVal[1]));
            if (Diff_Point_Proof[1][0] > Epsilon7 && Diff_Point_Proof[1][1] > Epsilon7) {
                Priority_Queue* emnt4 = new Priority_Queue;
                emnt4->Assign_Element_to_Queue(element1, emnt2, false, true, DMValue1);
                if (Prior_Queue.size() > 0) {
